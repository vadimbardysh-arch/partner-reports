"""
Generate a multi-store weekly HTML report for Coffee Lab by querying Databricks.
Produces coffee-lab/index.html — identical structure to the Coffee Lab dashboard,
...
COFFEE_LAB_PROVIDERS = {
    51785:  {"name": "Coffee Lab Залізнична", "short": "Залізнична", "city": "Lviv"},
    54035:  {"name": "Coffee Lab Угорська", "short": "Угорська", "city": "Lviv"},
    54036:  {"name": "Coffee Lab Малоголосківська", "short": "Малоголосківська", "city": "Lviv"},
    67783:  {"name": "Coffee Lab Шевченка", "short": "Шевченка", "city": "Lviv"},
    69011:  {"name": "Coffee Lab Пимоненка", "short": "Пимоненка", "city": "Lviv"},
    69514:  {"name": "Coffee Lab Чорновола", "short": "Чорновола", "city": "Lviv"},
    75845:  {"name": "Coffee Lab Садова", "short": "Садова", "city": "Lviv"},
    81626:  {"name": "Coffee Lab Чернівецька", "short": "Чернівецька", "city": "Lviv"},
    173134: {"name": "Coffee Lab вул. Гнатюка", "short": "Гнатюка", "city": "Lviv"},
}

PROVIDER_IDS = ",".join(str(k) for k in COFFEE_LAB_PROVIDERS)
...
def fetch_sponsored_listings(conn):
    """Fetch Sponsored Listing campaign data."""
    return query(conn, f"""
    SELECT
      c.provider_id,
      c.name AS campaign_name,
      ...
      CONCAT(YEAR(c.order_created_date), '-W', WEEKOFYEAR(c.order_created_date)) AS order_week,
      COUNT(*) AS orders,
      ROUND(SUM(c.discount_value_local), 0) AS total_spend_uah,
      ROUND(SUM(c.provider_spend_local), 0) AS provider_spend_uah
    FROM ng_public_spark.etl_delivery_campaign_order_metrics c
    WHERE c.provider_id IN ({PROVIDER_IDS})
      AND c.order_created_date >= DATE_SUB(CURRENT_DATE(), {WEEKS_BACK * 7})
      AND (LOWER(c.name) LIKE '%sponsored%' OR LOWER(c.spend_objective) LIKE '%sponsored%')
    GROUP BY c.provider_id, c.name, DATE(c.campaign_start), DATE(c.campaign_end),
             CONCAT(YEAR(c.order_created_date), '-W', WEEKOFYEAR(c.order_created_date))
    ORDER BY order_week DESC, orders DESC
    """)
...
def build_data(weekly_df, ops_df, items_df, orders_df, complaints_df, cancelled_df, revenue_df, campaigns_df,
    ...
    for _, r in weekly_df.iterrows():
        ...
        info = COFFEE_LAB_PROVIDERS.get(int(pid), {})
        stores_map[int(pid)] = {
    ...
    for row in orders_list:
        ...
        if pid and int(pid) in COFFEE_LAB_PROVIDERS:
            row["provider_short"] = COFFEE_LAB_PROVIDERS[int(pid)]["short"]
        else:
    ...
    for _, r in campaigns_df.iterrows():
        ...
        campaigns.append({
            ...
            "provider_id": pid,
            "provider_short": COFFEE_LAB_PROVIDERS.get(pid, {}).get("short", str(pid)),
    ...
    sp_status = {}
    for pid in COFFEE_LAB_PROVIDERS:
...
window.toggleDark = function() {{
  ...
  try {{ localStorage.setItem('coffee-lab-dark', isDark ? '1' : '0') }} catch(e) {{}}
  Chart.defaults.color = isDark ? '#D1D5DB' : '#374151';
  renderAll();
}};
(function() {{ try {{ if (localStorage.getItem('coffee-lab-dark') === '1') {{ document.body.classList.add('dark'); document.getElementById('theme-toggle').textContent = '☀️'; Chart.defaults.color = '#D1D5DB'; }} }} catch(e) {{}} }})();
...
def main():
    ...
    print(f"Starting Coffee Lab report generation at {generated_at}")
