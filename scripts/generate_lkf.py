"""
Generate a weekly HTML report for LKF (Вінниця) by querying Databricks.
Produces lkf/index.html — identical structure to the Ваш Лаваш dashboard.
"""
...
LKF_PROVIDERS = {
    164301: {"name": "LKF вул. Замостянська", "short": "Замостянська", "city": "Vinnytsia"},
}

PROVIDER_IDS = ",".join(str(k) for k in LKF_PROVIDERS)
...
def build_data(weekly_df, ops_df, items_df, orders_df, complaints_df, cancelled_df, revenue_df, campaigns_df, smart_promo_df=None):
    ...
    for _, r in weekly_df.iterrows():
        ...
        info = LKF_PROVIDERS.get(int(pid), {})
        stores_map[int(pid)] = {
    ...
    for row in orders_list:
        ...
        if pid and int(pid) in LKF_PROVIDERS:
            row["provider_short"] = LKF_PROVIDERS[int(pid)]["short"]
        else:
    ...
    for _, r in campaigns_df.iterrows():
        ...
        campaigns.append({
            ...
            "provider_id": pid,
            "provider_short": LKF_PROVIDERS.get(pid, {}).get("short", str(pid)),
...
window.toggleDark = function() {{
  ...
  try {{ localStorage.setItem('lkf-dark', isDark ? '1' : '0') }} catch(e) {{}}
  Chart.defaults.color = isDark ? '#D1D5DB' : '#374151';
  renderAll();
}};
(function() {{ try {{ if (localStorage.getItem('lkf-dark') === '1') {{ document.body.classList.add('dark'); document.getElementById('theme-toggle').textContent = '☀️'; Chart.defaults.color = '#D1D5DB'; }} }} catch(e) {{}} }})();
...
def main():
    ...
    print(f"Starting LKF report generation at {generated_at}")
