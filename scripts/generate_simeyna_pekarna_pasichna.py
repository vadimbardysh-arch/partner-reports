"""
Generate a weekly HTML report for Сімейна Пекарня вул. Пасічна (provider 202414).
Produces simeyna-pekarna-pasichna/index.html with Smart Promo & Sponsored Listings sections.
"""
...
SIMEYNA_PIZZA_PROVIDERS = {
    202414: {"name": "Сімейна Пекарня вул. Пасічна 85", "short": "Пасічна 85", "city": "Lviv", "brand": "Сімейна Пекарня"},
}
...
window.toggleDark = function() {{
  ...
  try {{ localStorage.setItem('simeyna-pekarna-pasichna-dark', isDark ? '1' : '0') }} catch(e) {{}}
  Chart.defaults.color = isDark ? '#D1D5DB' : '#374151';
  renderAll();
}};
(function() {{ try {{ if (localStorage.getItem('simeyna-pekarna-pasichna-dark') === '1') {{ document.body.classList.add('dark'); document.getElementById('theme-toggle').textContent = '☀️'; Chart.defaults.color = '#D1D5DB'; }} }} catch(e) {{}} }})();
...
def main():
    ...
    print(f"Starting Сімейна Пекарня Пасічна report generation at {generated_at}")
    ...
    out_dir = REPO_ROOT / "simeyna-pekarna-pasichna"
