"""
Generate a multi-store weekly HTML report for Epic Group (Epic / Cukor / TEDDY / Море Риби).
Produces epic-group/index.html — identical structure to the Vash Lavash dashboard.
"""
...
EPIC_GROUP_PROVIDERS = {
    31504:  {"name": "Epic Cheeseburger (Галицька)", "short": "Epic Галицька", "city": "Lviv"},
    187635: {"name": "Epic Cafe DL (Підстригача)", "short": "Epic Cafe DL", "city": "Lviv"},
    199279: {"name": "Epic Cheeseburger (Київ)", "short": "Epic Київ", "city": "Kyiv"},
    682702: {"name": "EPIC LEM (Вітовського)", "short": "EPIC LEM", "city": "Lviv"},
    31502:  {"name": "Cukor Red (Рогатинців)", "short": "Cukor Red", "city": "Lviv"},
    31503:  {"name": "Cukor Black (Крива Липа)", "short": "Cukor Black", "city": "Lviv"},
    31505:  {"name": "МОРЕ РИБИ (Рогатинців)", "short": "Море Риби", "city": "Lviv"},
    31506:  {"name": "TEDDY (Скорика)", "short": "TEDDY", "city": "Lviv"},
}
VASH_LAVASH_PROVIDERS = EPIC_GROUP_PROVIDERS  # alias for compatibility

PROVIDER_IDS = ",".join(str(k) for k in EPIC_GROUP_PROVIDERS)
...
<title>EPIC GROUP | тижневий звіт</title>
...
<header class="header">
  <div class="header-left">
    ...
    <h1>EPIC GROUP | тижневий звіт</h1>
  </div>
...
function renderKPIs() {{
  ...
  const opsSel = getOpsData()[selK] || {{}};
  let avgAvail = 0, avgAccept = 0, avCnt = 0, acCnt = 0;
  ids.forEach(id => {{
    const lo = opsSel[id] || (Object.keys(opsSel).length === 0 ? D.latest_ops[id] : null);
    if (lo) {{
      if (lo.availability != null) {{ avgAvail += lo.availability; avCnt++; }}
      if (lo.acceptance != null) {{ avgAccept += lo.acceptance; acCnt++; }}
    }}
  }});
  avgAvail = avCnt > 0 ? avgAvail / avCnt : 0;
  avgAccept = acCnt > 0 ? avgAccept / acCnt : 0;
...
function renderInsights() {{
  ...
  const opsSel2 = getOpsData()[selK] || {{}};
  let avgAvail = 0, avgAccept = 0, avCnt = 0, acCnt = 0, bad = 0, ord = 0;
  ids.forEach(id => {{
    const lo = opsSel2[id] || (Object.keys(opsSel2).length === 0 ? D.latest_ops[id] : null);
    if (lo) {{
      if (lo.availability != null) {{ avgAvail += lo.availability; avCnt++; }}
      if (lo.acceptance != null) {{ avgAccept += lo.acceptance; acCnt++; }}
    }}
...
window.toggleDark = function() {{
  ...
  try {{ localStorage.setItem('epic-group-dark', isDark ? '1' : '0') }} catch(e) {{}}
  Chart.defaults.color = isDark ? '#D1D5DB' : '#374151';
  renderAll();
}};
(function() {{ try {{ if (localStorage.getItem('epic-group-dark') === '1') {{ document.body.classList.add('dark'); document.getElementById('theme-toggle').textContent = '☀️'; Chart.defaults.color = '#D1D5DB'; }} }} catch(e) {{}} }})();
...
def main():
    ...
    print(f"Starting Epic Group report generation at {generated_at}")
