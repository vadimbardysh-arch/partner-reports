"""
Generate a multi-store weekly HTML report for Сімейна Пекарня (4 точки Львів).
Produces simeyna-pekarna/index.html with Smart Promo & Sponsored Listings sections.
"""
...
SIMEYNA_PIZZA_PROVIDERS = {
    174858: {"name": "Сімейна Пекарня вул. Патона 9", "short": "Патона 9", "city": "Lviv", "brand": "Сімейна Пекарня"},
    174859: {"name": "Сімейна Пекарня вул. Сихівська 7", "short": "Сихівська 7", "city": "Lviv", "brand": "Сімейна Пекарня"},
    180428: {"name": "Сімейна Пекарня вул. Руставелі 6", "short": "Руставелі 6", "city": "Lviv", "brand": "Сімейна Пекарня"},
    184847: {"name": "Сімейна Пекарня вул. Вашингтона 11", "short": "Вашингтона 11", "city": "Lviv", "brand": "Сімейна Пекарня"},
}
...
<title>СІМЕЙНА ПЕКАРНЯ | тижневий звіт</title>
...
<header class="header">
  <div class="header-left">
    ...
    <h1>СІМЕЙНА ПЕКАРНЯ | тижневий звіт</h1>
  </div>
...
window.toggleDark = function() {{
  ...
  try {{ localStorage.setItem('simeyna-pekarna-dark', isDark ? '1' : '0') }} catch(e) {{}}
  Chart.defaults.color = isDark ? '#D1D5DB' : '#374151';
  renderAll();
}};
(function() {{ try {{ if (localStorage.getItem('simeyna-pekarna-dark') === '1') {{ document.body.classList.add('dark'); document.getElementById('theme-toggle').textContent = '☀️'; Chart.defaults.color = '#D1D5DB'; }} }} catch(e) {{}} }})();
...
def main():
    ...
    print(f"Starting Сімейна Пекарня report generation at {generated_at}")
    ...
    out_dir = REPO_ROOT / "simeyna-pekarna"
