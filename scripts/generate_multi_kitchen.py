"""
Generate a multi-brand weekly HTML report for Multi-Kitchen by querying Databricks.
Produces multi-kitchen/index.html — identical structure to the Ваш Лаваш dashboard,
plus Smart Promo & Sponsored Listings sections.
"""
...
MULTI_KITCHEN_PROVIDERS = {
    79017:  {"name": "ШАВУХА ТОПчик Хрещатик", "short": "Шавуха Хрещатик", "city": "Kyiv", "brand": "ШАВУХА ТОПчик"},
    79018:  {"name": "ШАВУХА ТОПчик Васильківська", "short": "Шавуха Васильків.", "city": "Kyiv", "brand": "ШАВУХА ТОПчик"},
    79019:  {"name": "ШАВУХА ТОПчик Борщагівка", "short": "Шавуха Борщагів.", "city": "Kyiv", "brand": "ШАВУХА ТОПчик"},
    128521: {"name": "ШАВУХА ТОПчик Макіївська", "short": "Шавуха Макіївська", "city": "Kyiv", "brand": "ШАВУХА ТОПчик"},
    79106:  {"name": "БОРЩ Хрещатик", "short": "Борщ Хрещатик", "city": "Kyiv", "brand": "БОРЩ"},
    79108:  {"name": "БОРЩ Борщагівка", "short": "Борщ Борщагівка", "city": "Kyiv", "brand": "БОРЩ"},
    128524: {"name": "БОРЩ Макіївська", "short": "Борщ Макіївська", "city": "Kyiv", "brand": "БОРЩ"},
    159537: {"name": "БОРЩ Луцьк", "short": "Борщ Луцьк", "city": "Lutsk", "brand": "БОРЩ"},
    90216:  {"name": "TOP BURGER Хрещатик", "short": "Burger Хрещатик", "city": "Kyiv", "brand": "TOP BURGER"},
    90217:  {"name": "TOP BURGER Борщагівка", "short": "Burger Борщагів.", "city": "Kyiv", "brand": "TOP BURGER"},
    128508: {"name": "TOP BURGER Макіївська", "short": "Burger Макіївська", "city": "Kyiv", "brand": "TOP BURGER"},
    159764: {"name": "Yum Yum Asia Луцьк", "short": "YYA Луцьк", "city": "Lutsk", "brand": "Yum Yum Asia"},
    159783: {"name": "Yum Yum Asia Вінграновського", "short": "YYA Вінгранов.", "city": "Kyiv", "brand": "Yum Yum Asia"},
    159786: {"name": "Yum Yum Asia Миколайчука", "short": "YYA Миколайчука", "city": "Kyiv", "brand": "Yum Yum Asia"},
    159787: {"name": "Yum Yum Asia Макіївська", "short": "YYA Макіївська", "city": "Kyiv", "brand": "Yum Yum Asia"},
    159789: {"name": "Yum Yum Asia Хрещатик", "short": "YYA Хрещатик", "city": "Kyiv", "brand": "Yum Yum Asia"},
}

PROVIDER_IDS = ",".join(str(k) for k in MULTI_KITCHEN_PROVIDERS)

CITY_UA = {"Kyiv": "Київ", "Lutsk": "Луцьк"}
...
def fetch_smart_promo_status(conn):
    return query(conn, f"""
    SELECT
      provider_id,
      ...
      DATE(smart_promo_provider_enrollment_start_ts) AS enroll_start,
      DATE(smart_promo_provider_enrollment_end_ts) AS enroll_end
    FROM core_models_spark.fact_provider_smart_promo_offer_campaign_enrollment
    WHERE provider_id IN ({PROVIDER_IDS})
      AND smart_promo_provider_enrollment_end_ts >= CURRENT_DATE()
      AND smart_promo_enrollment_state = 'active'
    """)
...
def fetch_sponsored_listings(conn):
    return query(conn, f"""
    SELECT
      provider_id, state, offer_id, ad_id, city_id,
      DATE(start) AS start_date,
      DATE(end) AS end_date
    FROM ng_delivery_spark.delivery_paid_visibility_signup
    WHERE provider_id IN ({PROVIDER_IDS})
    ORDER BY provider_id, start DESC
    """)
...
SPEND_OBJ_UA = {
    ...
    "retention": "Утримання",
    "sp_activation": "Активація",
    "sp_reactivation": "Реактивація",
    "sp_engagement": "Залучення",
    "sp_fully_funded": "Повне фінансування",
    "engagement": "Залучення",
}
TARGET_UA = {"delivery_price": "Доставка", "item_price": "Знижка на товар"}
PROMO_TYPE_UA = {"double_deal": "Подвійна пропозиція", "regular": "Звичайна"}
...
def build_data(weekly_df, ops_df, items_df, orders_df, complaints_df,
    ...
    for row in orders_list:
        ...
        if pid and int(pid) in MULTI_KITCHEN_PROVIDERS:
            row["provider_short"] = MULTI_KITCHEN_PROVIDERS[int(pid)]["short"]
        else:
    ...
    for _, r in campaigns_df.iterrows():
        ...
        campaigns.append({
            ...
            "provider_id": pid,
            "provider_short": MULTI_KITCHEN_PROVIDERS.get(pid, {}).get("short", str(pid)),
    ...
    # Smart Promo per provider
    sp_summary = {}
    if len(smart_promo_df):
        for _, r in smart_promo_df.iterrows():
            pid = int(to_native(r["provider_id"]))
            if pid not in sp_summary:
                sp_summary[pid] = {"types": set(), "objectives": set(), "active": True}
            sp_summary[pid]["types"].add(str(r["smart_promo_offer_type"] or ""))
            sp_summary[pid]["objectives"].add(str(r["campaign_spend_objective"] or ""))
    sp_data = {}
    for pid, v in sp_summary.items():
        sp_data[pid] = {
            "active": True,
            "types": sorted(v["types"]),
            "objectives": sorted(v["objectives"]),
        }
    data["smart_promo"] = sp_data

    # Sponsored Listings per provider
    sl_data = {}
    if len(listings_df):
        today_str = datetime.utcnow().strftime("%Y-%m-%d")
        for _, r in listings_df.iterrows():
            pid = int(to_native(r["provider_id"]))
            end_str = str(r["end_date"])
            state = str(r["state"] or "")
            is_active = (end_str >= today_str) and state == "active"
            if pid not in sl_data:
                sl_data[pid] = {"active": False, "history": []}
            if is_active:
                sl_data[pid]["active"] = True
            sl_data[pid]["history"].append({
                "state": state,
                "start": str(r["start_date"]),
                "end": end_str,
            })
        for pid in sl_data:
            sl_data[pid]["history"] = sl_data[pid]["history"][:5]
...
<title>Мультибренд Кухня | тижневий звіт</title>
...
.badge-green{{background:rgba(16,185,129,.1);color:var(--pos)}}
.badge-red{{background:rgba(239,68,68,.1);color:var(--neg)}}
...
.promo-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;margin-top:12px}}
.promo-card{{background:var(--card);border-radius:var(--r);box-shadow:var(--shadow);border:1px solid var(--border);padding:16px}}
.promo-card h4{{font-size:13px;font-weight:700;margin-bottom:4px}}
.promo-card .promo-brand{{font-size:11px;color:var(--text2);margin-bottom:8px}}
.promo-card .promo-row{{display:flex;justify-content:space-between;font-size:12px;padding:3px 0}}
.promo-card .promo-label{{color:var(--text2)}}
...
body.dark .promo-card{{background:var(--card)}}
...
<header class="header">
  <div class="header-left">
    ...
    <h1>МУЛЬТИБРЕНД КУХНЯ | тижневий звіт</h1>
  </div>
  <div class="header-right">
    <div class="ms-wrap" id="brand-ms"><button class="ms-btn" id="brand-btn">Всі бренди</button><div class="ms-panel" id="brand-panel"></div></div>
...
<nav class="main-nav" id="main-nav">
  ...
  <a href="#promo-section" class="nav-link">Smart Promo & Listing</a>
...
<main class="main-content">
  ...
  <section id="promo-section" class="section">
    ...
    <div class="section-insight" id="promo-insight"></div>
...
function populateBrandFilter() {{
  ...
  const brands = [...new Set(Object.values(D.stores).map(s => s.brand))].sort();
  const items = brands.map(b => ({{ value: b, label: b }}));
  buildMsPanel(panel, items, selectedBrands, 'Всі бренди', function() {{
    updateMsLabel(btn, selectedBrands, 'Всі бренди', v => v);
...
function populateStoreFilter() {{
  ...
  let cityIds = Object.keys(D.stores).map(Number);
  if (selectedBrands.size > 0) cityIds = cityIds.filter(id => selectedBrands.has(D.stores[id].brand));
  if (selectedCities.size > 0) cityIds = cityIds.filter(id => selectedCities.has(D.stores[id].city_en));
  const items = cityIds
    .sort((a, b) => (D.stores[a].short || '').localeCompare(D.stores[b].short || '', 'uk'))
    .filter(id => D.stores[id])
    .map(id => ({{ value: String(id), label: D.stores[id].short + ' (' + D.stores[id].brand + ')' }}));
...
function getFilteredStoreIds() {{
  ...
  let ids = Object.keys(D.stores).map(Number);
  if (selectedBrands.size > 0) ids = ids.filter(id => selectedBrands.has(D.stores[id].brand));
  if (selectedCities.size > 0) ids = ids.filter(id => selectedCities.has(D.stores[id].city_en));
  return ids;
}}
...
function renderStoresTable() {{
  ...
  t += '<tr class="total-row"><td></td><td colspan="3">Всього</td><td class="text-right">' + totOrd + '</td><td colspan="5"></td><td class="text-right">' + totalBadRate.toFixed(1) + '%</td></tr>';
  t += '</tbody></table>';
...
function renderRevenueChart() {{
  ...
  ids.filter(id => rw[id]).sort((a, b) => (rw[b].net_income || 0) - (rw[a].net_income || 0)).forEach(id => {{
    const r = rw[id]; const s = D.stores[id];
...
function renderCampaignsChart() {{
  ...
  chartInstances['chart-campaign-spend'] = new Chart(document.getElementById('chart-campaign-spend'), {{
    ...
    options: {{ responsive: true, maintainAspectRatio: false,
      plugins: {{ legend: {{ position: 'bottom', labels: {{ usePointStyle: true, padding: 12, font: {{ size: 11 }} }} }} }},
      scales: {{ x: {{ grid: {{ display: false }} }}, y: {{ beginAtZero: true, position: 'left', grid: {{ color: 'rgba(0,0,0,.05)' }}, ticks: {{ callback: v => '₴' + v.toLocaleString('uk-UA') }} }}, y1: {{ beginAtZero: true, position: 'right', grid: {{ display: false }}, ticks: {{ font: {{ size: 10 }} }} }} }}
    }}
  }});
...
function renderCampaigns() {{
  ...
  rows.forEach(r => {{
    ...
    bycamp[key].orders += r.orders; bycamp[key].total_discount += r.total_discount;
    bycamp[key].bolt_spend += r.bolt_spend; bycamp[key].provider_spend += r.provider_spend;
...
function renderPromoSection() {{
  ...
  const sl = D.sponsored_listings || {{}};

  let spActive = 0, spInactive = 0, slActive = 0, slInactive = 0;
  ids.forEach(id => {{
    if (sp[id] && sp[id].active) spActive++; else spInactive++;
    if (sl[id] && sl[id].active) slActive++; else slInactive++;
  }});

  let insight = '<b>Smart Promo:</b> активний на <b class="insight-good">' + spActive + '</b> закладах';
  if (spInactive > 0) insight += ', неактивний на <b class="insight-bad">' + spInactive + '</b>';
  insight += '. <b>Sponsored Listing:</b> активний на <b class="insight-good">' + slActive + '</b> закладах';
  if (slInactive > 0) insight += ', неактивний на <b class="insight-bad">' + slInactive + '</b>';
  insight += '.';
  document.getElementById('promo-insight').innerHTML = insight;

  let html = '';
  ids.sort((a, b) => (D.stores[a].short || '').localeCompare(D.stores[b].short || '', 'uk')).forEach(id => {{
    const s = D.stores[id];
    const spd = sp[id];
    const sld = sl[id];
    const spStatus = spd && spd.active;
    const slStatus = sld && sld.active;

    html += '<div class="promo-card">';
    html += '<h4>' + s.short + '</h4>';
    html += '<div class="promo-brand">' + (s.brand || '') + ' · ' + s.city + '</div>';

    html += '<div class="promo-row"><span class="promo-label">Smart Promo</span>';
    if (spStatus) {{
      html += '<span class="badge badge-green">✓ Активний</span>';
    }} else {{
      html += '<span class="badge badge-red">✕ Неактивний</span>';
    }}
    html += '</div>';

    if (spd && spd.types && spd.types.length) {{
      const typeMap = {{'double_deal': 'Подвійна пропозиція', 'regular': 'Звичайна'}};
      html += '<div class="promo-row"><span class="promo-label" style="font-size:11px">Типи</span><span style="font-size:11px">' + spd.types.map(t => typeMap[t] || t).join(', ') + '</span></div>';
    }}

    html += '<div class="promo-row"><span class="promo-label">Sponsored Listing</span>';
    if (slStatus) {{
      html += '<span class="badge badge-green">✓ Активний</span>';
    }} else {{
      html += '<span class="badge badge-red">✕ Неактивний</span>';
    ...
    if (sld && sld.history && sld.history.length) {{
      const last = sld.history[0];
      html += '<div class="promo-row"><span class="promo-label" style="font-size:11px">Останній період</span><span style="font-size:11px">' + last.start + ' → ' + last.end + ' (' + last.state + ')</span></div>';
    }}

    html += '</div>';
  }});
  document.getElementById('promo-grid').innerHTML = html || '<p style="color:var(--text2);text-align:center;padding:24px">Немає даних.</p>';
}}
...
function renderBudgetCalc() {{
  ...
  ids.forEach(id => {{
    const s = D.stores[id]; if (!s) return;
    ...
    html += '<div class="calc-store"><div class="calc-store-name">' + s.short + '</div><div class="calc-store-city">' + (s.brand || '') + ' · ' + s.city + '</div>';
    html += '<div class="calc-bar-wrap"><div class="calc-bar" style="width:' + pct.toFixed(1) + '%;background:' + barColor + '"></div></div>';
    html += '<div class="calc-metrics"><span class="calc-spent" style="color:var(--neg)">Витрачено: ₴' + spent.toLocaleString('uk-UA') + '</span><span class="calc-left" style="' + leftColor + '">Залишок: ₴' + left.toLocaleString('uk-UA') + '</span></div></div>';
  }});
  html += '</div>';
...
function renderTopItems() {{
  ...
  ids.forEach(id => {{
    ...
    const s = D.stores[id];
    html += '<div class="items-card"><h4>' + s.short + '</h4><div class="items-city">' + (s.brand || '') + ' · ' + s.city + '</div><ol>';
    items.forEach(it => {{
...
window.toggleDark = function() {{
  ...
  try {{ localStorage.setItem('multi-kitchen-dark', isDark ? '1' : '0') }} catch(e) {{}}
  Chart.defaults.color = isDark ? '#D1D5DB' : '#374151';
  renderAll();
}};
(function() {{ try {{ if (localStorage.getItem('multi-kitchen-dark') === '1') {{ document.body.classList.add('dark'); document.getElementById('theme-toggle').textContent = '☀️'; Chart.defaults.color = '#D1D5DB'; }} }} catch(e) {{}} }})();
...
initMsToggle('brand-btn', 'brand-panel');
...
def main():
    ...
    print(f"Starting Мультибренд Кухня report generation at {generated_at}")
    ...
    try:
        ...
        smart_promo_df = fetch_smart_promo_status(conn)
